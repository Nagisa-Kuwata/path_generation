...
impl LrfScan {
    /// Returns `true` if `target` lies on at least one of the actual 720 LRF
    /// rays captured in this scan before that ray terminates.
    ///
    /// This uses the same fixed 0.5-degree ray set as [`Lrf::scan`].  Unlike
    /// an arbitrary line-of-sight check, it answers the stricter question:
    /// "would the real LRF have detected this cell in this scan?"
    pub fn detects_cell(&self, target: GridPos) -> bool {
        let origin_gp = GridPos::from(self.origin);
        if origin_gp.col == target.col && origin_gp.row == target.row {
            return true;
        }

        let rx = self.origin.x / CELL_SIZE + 120.5;
        let ry = 120.5 - self.origin.y / CELL_SIZE;
        let target_col = target.col as i32;
        let target_row = target.row as i32;

        for i in 0..self.points.len() {
            let endpoint = self.points[i];
            let edx = endpoint.x - self.origin.x;
            let edy = endpoint.y - self.origin.y;
            let endpoint_dist = (edx * edx + edy * edy).sqrt();
            if endpoint_dist < 1e-7 {
                continue;
            }
            ...
            let total_cells = endpoint_dist / CELL_SIZE;
